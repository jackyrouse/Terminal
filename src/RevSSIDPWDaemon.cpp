	...
	while (1)
	{
		...
		else
		{
			...
			else
			{
				...
				memset(UNTIE_buf, 0x00, 33);
				memset(RUN1_buf, 0x00, 33);
				memset(RUN2_buf, 0x00, 33);
				...
				if(reader.parse(ssidandpwstr,value))
				{
					...
					strcpy(RUN1_buf, value["RUN1"].asString().c_str());
					strcpy(UNTIE_buf, value["UNTIE"].asString().c_str());
					strcpy(RUN2_buf, value["RUN2"].asString().c_str());
				}

				if(strcmp(RUN1_buf, "yes"))
				{
				...
				if(strcmp(RUN2_buf, "yes"))
				{
					fprintf(stdout, "UNTIE is : %s\n", UNTIE_buf);
					//RevDaemon->tcpclientptr->AddToSendQueue();
				}
